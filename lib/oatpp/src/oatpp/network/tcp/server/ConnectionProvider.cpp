...
oatpp::v_io_handle ConnectionProvider::instantiateServer(){
  ...
  auto portStr = oatpp::utils::conversion::int32ToStr(m_hostAddress.port);

  ret = getaddrinfo(m_hostAddress.host->c_str(), portStr->c_str(), &hints, &result);
  if (ret != 0) {
