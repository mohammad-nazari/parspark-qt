...
namespace anar::service {
   AFromJsonVisitor::AFromJsonVisitor(nlohmann::ordered_json& jsonNlohmann)
       : m_json(jsonNlohmann) {
   }
   ...
   bool AFromJsonVisitor::visit(model::ErrorModel& error) {
      try {
         ...
         std::shared_ptr<AFromJsonVisitor> aFromJsonVisitor = std::make_shared<AFromJsonVisitor>(m_json["Level"]);
         ...
         for (auto& json : m_json["SubErrors"]) {
            aFromJsonVisitor = std::make_shared<AFromJsonVisitor>(std::ref(json));
