...
namespace anar::parspark::controller {
    ...
    bool LoginController::DoLogin(const model::LoginModel &login) {
        try {
            ...
            anar::common::database::DatabasePtr db = anar::common::database::DatabaseFactory::Create(
                    anar::common::constant::DatabaseEngines::ANAR_MYSQL.Name, login);
            if (db) {
                if (db->Connect()) {
                    odb::query<model::UserModel> query(odb::query<model::UserModel>::UserName == login.UserName &&
                                                                                                 odb::query<model::UserModel>::PassWord ==
                                                                                                 passWord);
//                    odb::query<model::UserModel> query(odb::query<model::UserModel>::Id == 1);
                    ...
                    if (db->Select<model::UserModel>(query, userList)) {
                        if (userList.empty()) {
                            m_error = anar::common::service::ErrorManager::GenerateError(1,
                                                                                         anar::common::constant::ErrorLevel::ANAR_HIGH_ERROR,
                                                                                         "User name '" +
                                                                                         login.UserName +
                                                                                         "' or password '" +
                                                                                         login.PassWord + "(" +
...
//                    }
            ...
            m_error = anar::common::service::ErrorManager::GenerateError(1,
                                                                         anar::common::constant::ErrorLevel::ANAR_HIGH_ERROR,
                                                                         "DB Error: " + std::string(exception.what()));
        }
