...
namespace anar::parspark::service {
    ...
    bool ToJsonVisitor::Visit(model::UserModel &user) {
        try {
            ...
            if (!user.Person->Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'person' json data.");
            ...
            if (!user.CreatingUser->Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Creating User' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            };
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["UpdatingUser"]);
            if (!user.UpdatingUser->Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'updating user' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            };
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'user' json data: " + std::string(exception.what()));
    ...
    bool ToJsonVisitor::Visit(model::LoginModel &login) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'login' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::AlprByCameraModel &alprByCamera) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'ALPR By Camera' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::AlprModel &alpr) {
        try {
            ...
            if (!alpr.ByCamera.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'By Camera' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'ALPR' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::PriceBoardModel &priceBoard) {
        try {
            ...
            if (!priceBoard.Address.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Address' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Price Board' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::CapacityBoardModel &capacityBoard) {
        try {
            ...
            if (!capacityBoard.ComPortAddress.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Com Port Address' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["ServerAddress"]);
            if (!capacityBoard.ServerAddress.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Server Address' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            m_json["ParkingName"] = capacityBoard.ParkingName;
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Capacity Board' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::BoardModel &board) {
        try {
            ...
            if (!board.PriceBoard.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Price Board' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["CapacityBoard"]);
            if (!board.CapacityBoard.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Capacity Board' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Board' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::CameraInfoModel &cameraInfo) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Camera Info' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::CameraModel &camera) {
        try {
            ...
            if (!camera.EnterCamera.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Enter Camera' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["ExitCamera"]);
            if (!camera.ExitCamera.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Exit Camera' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Camera' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::CardReaderModel &cardReader) {
        try {
            ...
            if (!cardReader.EnterCardReader.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Enter Card Reader' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["ExitCardReader"]);
            if (!cardReader.ExitCardReader.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Exit Card Reader' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Card Reader' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::ComPortModel &comPort) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'COM Port' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::GateInfoModel &gateInfo) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Gate Info' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::GateModel &gate) {
        try {
            ...
            if (!gate.EnterGate.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Enter Gate' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["ExitGate"]);
            if (!gate.ExitGate.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Exit Gate' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
    ...
    bool ToJsonVisitor::Visit(model::TariffModel &tariff) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Tariff' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::ParkingModel &parking) {
        try {
            ...
            if (!parking.DayTariff.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Day Tariff' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["NightTariff"]);
            if (!parking.NightTariff.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Night Tariff' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
    ...
    bool ToJsonVisitor::Visit(model::PaymentInfoModel &paymentInfo) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Payment Info' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::PaymentModel &payment) {
        try {
            ...
            if (!payment.CitizenDevice.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Citizen Device' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["PosDevice"]);
            if (!payment.PosDevice.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'POS Device' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Payment' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::PrinterModel &printer) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Printer' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::ServerModel &server) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Server' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::SettingsModel &settings) {
        try {
            ...
            if (!settings.Alpr.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'ALPR' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["Board"]);
            if (!settings.Board.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Board' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["Camera"]);
            if (!settings.Camera.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Camera' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["CardReader"]);
            if (!settings.CardReader.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Card Reader' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["DataBase"]);
            if (!settings.DataBase.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'DataBase' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["Gate"]);
            if (!settings.Gate.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Gate' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["Parking"]);
            if (!settings.Parking.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Parking' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["Payment"]);
            if (!settings.Payment.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Payment' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["Software"]);
            if (!settings.Software.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Software' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
            toJsonVisitor = std::make_shared<ToJsonVisitor>(m_json["Printer"]);
            if (!settings.Printer.Accept(*toJsonVisitor)) {
                m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Printer' json data.");
                m_error->SubErrors.emplace_back(*toJsonVisitor->Error());
                return false;
            }
        } catch (std::exception &exception) {
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Settings' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
    ...
    bool ToJsonVisitor::Visit(model::SoftwareModel &software) {
        try {
            ...
            m_error = anar::service::ErrorManager::GenerateErrorPtr(1, anar::constant::ErrorLevel::ANAR_HIGH_WARNING, "Error on write 'Software' json data: " + std::string(exception.what()));
            LOG(INFO) << m_error->Message.c_str();
            return false;
        }
        return true;
